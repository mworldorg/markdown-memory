...
### Шаг 3.9. Scrub секретов перед записью и отдачей (hard-stop, строже handoff)

> **Вывод этого скилла пользователь руками вставляет в claude.ai** (внешний сервис) — это самая «утечкоёмкая» точка mm. Промпт собирается из паспорта + контекста чата, куда мог просочиться токен. Поэтому политика строже, чем у `mm-handoff`.

Переиспользуй единый механизм — паттерны бери из канона `<repo>/config/secret-patterns.json` (политику `mm-bridge` и пояснение см. в `<repo>/docs/SECRET-PATTERNS.md`, строка `mm-bridge` в «Политики применения»), не дублируй их здесь. Прогони **весь** собранный текст промпта (frontmatter + тело) через паттерны. TG-summary (Шаг 4.5) выводится из уже очищенного промпта → наследует маски, отдельно сканировать не нужно:

1. **Маскируй И Класс A, И Класс B** — типизированными плейсхолдерами с сохранением контекста (в отличие от handoff/save, здесь Класс B тоже маскируется, не warn-only).
2. **Если найдено что-либо (A или B) — жёсткий стоп.** НЕ записывай `next-prompt.md`, НЕ показывай вывод, НЕ отправляй TG. Выведи список находок (тип + место + первые 12 символов) и спроси одной строкой: `🛑 Найдено N потенциальных секретов, замаскировано. Отдать промпт с масками? (y / отмена)`.
3. Отдавай (Шаги 4–5) **только после явного `y`** — и только уже замаскированный текст. На «отмена» — ничего не пиши, скажи где посмотреть/почистить источник.
4. Если находок ноль — продолжай молча (Шаг 4), без лишнего шума.
